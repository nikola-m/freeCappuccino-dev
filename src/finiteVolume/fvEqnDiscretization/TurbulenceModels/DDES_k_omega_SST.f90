module DDES_k_omega_SST
!
! Implementation of Delayed Detached Eddy Simulation (DDES) 
...
! definition by approximately 0.43%, and the second is lower by less than 0.08%. 
  ...
  real(dp), parameter :: cd1 = 20.0_dp
  integer, parameter :: cd2 = 3
  ...
  public :: modify_viscosity_DDES_k_omega_sst
  public :: modify_viscosity_inlet_DDES_k_omega_sst
...
!***********************************************************************
!
subroutine modify_viscosity_DDES_k_omega_sst
!
!***********************************************************************
!
...
!***********************************************************************
  ...
  if(.not.allocated(hmax)) then
    ...
    write(*,'(a)') ' '
    write(*,'(a)') '  **Allocated hmax for DDES.'
    write(*,'(a)') ' '
...
!
  ...
  real(dp) :: wlog,wvis
  real(dp) :: cdes,lenles,lenrans,rd,fd,lDDES
  ! real(dp) :: W_S,Ri,F4
...
!
  ...
  do inp=1,numCells
    ...
    lenles = cdes*hmax(inp)
    lenrans = sqrt(te(inp))/(bettast*ed(inp))

    vist = (vis(inp)-viscos)/den(inp)
    rd = ( vist + viscos/den(inp) ) &
       / ( CAPPA**2 * walldistance(inp)**2 * sqrt( 0.5*magStrain(inp)*Vorticity(inp) ) + small )
    fd = 1.0-tanh( (cd1*rd)**cd2 )

    ! *** lDDES - DDES length scale ***
    lDDES = lenrans - fd*max(0., lenrans-lenles)

    sp(inp)=den(inp)*sqrt(te(inp))*vol(inp) / (lDDES+small) ! <- k^3/2 / k = k, dividing by k when moving to the lhs.    
...
subroutine modify_viscosity_inlet_DDES_k_omega_sst
!

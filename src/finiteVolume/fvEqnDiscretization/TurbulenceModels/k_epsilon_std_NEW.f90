...
! Initialize coef and source arrays
  ...
  !+------------------------------------------------------+
  ! COMPUTE STANDARD PRODUCTION - GEN
  !+------------------------------------------------------+
  ...
  do inp = 1,numCells
    ...
    su(inp) = su(inp) + urfm*a(diag(inp))*te(inp)
...
! Report range of scalar values and clip if negative
  ...
  write(6,'(2x,es11.4,3a,es11.4)') fimin,' <= TE <= ',fimax
...
subroutine modify_mu_eff_inlet_k_epsilon_std
!
! Purpose:
!   Update effective viscosity for standard k-epsilon model at inlet.
!
!
  use types
  use parameters

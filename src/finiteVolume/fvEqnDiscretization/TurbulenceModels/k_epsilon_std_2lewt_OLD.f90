    ...
    elseif ( bctype(ib) == 'wall') then
      ...
      do i=1,nfaces(ib)
        ...
        !
          ...
          ! 1) First substract the standard production from source term
          su(ijp)=su(ijp)-gen(ijp)*vol(ijp)

          ! 2) Calculate production for wall adjecent cell
          gen(ijp)=abs(tau(iWall))*cmu25*sqrt(te(ijp))/(dnw(iWall)*cappa)

          ! 3) Add this production to source vector
          su(ijp)=su(ijp)+gen(ijp)*vol(ijp)
...
!
  ...
  do inp=1,numCells
    ...
    ! Store old value of effective viscosity
    visold=vis(inp)
  ...
  do ib=1,numBoundaries
    ...
    if ( bctype(ib) == 'inlet' .or. bctype(ib) == 'outlet' .or. &
         bctype(ib) == 'symmetry'.or. bctype(ib) == 'empty') then
